# Contributing to dsh-maestro-config-lib

Thank you for contributing to **dsh-maestro-config-lib** (`@ddtcorex/dsh-maestro-config-lib`) — Maestro settings store library (atomic namespaced JSON store shared by dsh-maestro-* plugins, embedded dependency with no Cordis row).

## Getting Started

1. **Fork and clone** `github.com/ddtcorex/dsh-maestro-config-lib`.
...
3. Build the library (TypeScript → `lib/`):

   ```bash
   pnpm build        # runs tsc -p tsconfig.json
   ```

4. Open the project in your editor. Source lives at `src/index.ts`; tests at `tests/`.
...
- **Scope:** optional — e.g. `feat(store):`, `fix(migration):`, `docs(readme):`
...
pnpm build       # tsc -p tsconfig.json — ensures lib/ is not stale
```
...
- Security vulnerabilities: use GitHub's private advisory reporting at `https://github.com/ddtcorex/dsh-maestro-config-lib/security/advisories` — do not file a public issue.
